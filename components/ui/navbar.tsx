  ...
  return (
    <nav className="system-panel p-2 md:p-4 fixed bottom-0 left-0 right-0 md:left-4 md:top-4 md:bottom-4 md:w-64 flex md:flex-col gap-1 z-10">
      <div className="hidden md:flex items-center justify-center p-2 mb-6">
        <h1 className={`text-xl font-bold text-primary tracking-wider ${shouldFlicker('title') ? 'flickering-text' : ''}`}>DERELICT DAWN</h1>
      </div>
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`flex items-center gap-2 p-3 rounded-md transition-colors relative
                ${isActive 
                  ? "bg-accent/30 text-primary border border-primary/40"
                  : "hover:bg-accent/10 text-muted-foreground hover:text-primary"
                }`}
            >
              <item.icon className={`h-6 w-6 ${isReactor && shouldFlicker('reactor') ? 'flickering-text' : ''}`} />
              <span className="hidden md:inline text-lg">{item.name}</span>
              
              {/* Notification indicator for unread logs */}
              {isLogs && unreadLogsCount > 0 && (
                <span className="ml-auto mr-1 bg-red-500 text-white text-sm rounded-full h-6 w-6 flex items-center justify-center">
      ...
      <div className="hidden md:flex flex-col mt-auto mb-4">
        <div className="text-lg text-primary font-semibold mb-3">RESOURCES</div>
